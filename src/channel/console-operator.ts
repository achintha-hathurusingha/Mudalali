...
export class ConsoleOperatorChannel implements Channel {
  ...
  async sendDocument(
    ...
    if (isOperator(jid, this.operatorIdentities())) {
      process.stdout.write(`\n[file to you] ${filePath}${caption ? ` - ${caption}` : ""}\n`);
      return;
    }
    if (!this.inner.sendDocument) throw new Error(`${this.inner.name} cannot send files`);
    await this.inner.sendDocument(jid, filePath, fileName, caption);
  }
